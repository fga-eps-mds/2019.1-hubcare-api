...
from community.serializers.code_of_conduct_serializer import CodeofConductSerializer
...
class CodeOfConductView(APIView):
    ...
    def get(self, request, owner, repo):
        code_of_conduct = CodeOfConduct.objects.all().filter(owner=owner, repo=repo)
        ...
        if(code_serialized.data == []):
            ...
            if(github_request.status_code == 200):
                CodeOfConduct.objects.create(owner=owner, repo=repo, code_of_conduct=True, date=date.today())
            else:
                CodeOfConduct.objects.create(owner=owner, repo=repo, code_of_conduct=False, date=date.today())
