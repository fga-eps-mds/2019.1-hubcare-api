...
def test_exists_in_db(self):
        ...
        request = self.factory.get('/issues/help_wanted/fga-eps-mds/2019.1-hubcare-api')
        response = HelpWantedView.as_view()(request, 'jaco', 'teste')
        self.assertEqual(response.status_code, 200)

    @mock.patch('issues.views.help_wanted_views.requests.get',
                side_effect=mocked_requests_get)
    def test_help_wanted_exists(self, mock_get):
        
        request = self.factory.get('issues/help_wanted/test/repo_test')
        response = HelpWantedView.as_view()(request, 'test', 'repo_test')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data['owner'], 'test')
        self.assertEqual(response.data['repo'], 'repo_test')
        self.assertEqual(response.data['total_issues], '10')
        self.assertEqual(response.data['help_wanted_issues], '1')

    @mock.patch('issues.views.help_wanted_views.requests.get',
                side_effect=mocked_requests_get)
    def test_help_wanted_not_exists(self, mock_get):
        
        request = self.factory.get('issues/help_wanted/test/repo_test')
        response = HelpWantedView.as_view()(request, 'test', 'no_help_wanted')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data['owner'], 'test')
        self.assertEqual(response.data['repo'], 'no_help_wanted')
        self.assertEqual(response.data['total_issues], '10')
        self.assertEqual(response.data['help_wanted_issues], '1')
    ...
    def test_check_datetime_out(self, mock_get):
        ...
        '''
        request = self.factory.get('issues/help_wanted/test/repo_test')
        response = HelpWantedView.as_view()(request, 'jaco', 'mds')
        self.assertEqual(response.status_code, 404)
    ...
    def test_check_datetime(self, mock_get):
        ...
        request = self.factory.get('issues/help_wanted/test/repo_test')
        response = HelpWantedView.as_view()(request, 'test', 'old_help_wanted')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data['owner'], 'test')
        self.assertEqual(response.data['repo'], 'old_help_wanted')
        self.assertEqual(response.data['total_issues], '10')
        self.assertEqual(response.data['help_wanted_issues], '1')
